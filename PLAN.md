...
sections below are the backlog a stage is drawn from while S37 waits.
...
through S57 — every number this plan has spent but S37. A number is never
reissued, so a
...
- [ ] **The exit's own containers.** Three sites on paths a destructor or the
  thread's exit reaches allocate through the global allocator:
  `static_block::tear_down` fills a `Vec<*mut RcHeader>` through
  `object::sever_counted_slots`; `static_block::ll_static_block_register`
  grows the registry's `Vec`; `buffer_arena::with_buffer_arena` boxes the
  thread's `BufferArena` on the first long-lived payload, which
  `weak::table::draw` can reach from a step-4 destructor. None is on a
  collection frame, so the deny run does not see them. What closes each is
  the reset's shape: a chain in memory the layer already holds, and a refusal
