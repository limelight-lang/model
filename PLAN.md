...
- [ ] **A quiet thread's garbage is taken after X.** Edmond, 2026-09-18: the
  ...
  Today it does not: a collector serves a mutator whose R holds
  `worker::SOFT_THRESHOLD` (64) records or more, the round's threshold is
  constant in the working build (`worker::threshold_for_rounds`), the timer
  ...
  lane's re-offer — so a thread below the threshold, under no pressure and
  making no explicit fire, keeps its cycle garbage until it crosses the
  threshold or exits
  (`cycle::collect::tests::what_the_byte_arms::`
  `an_unarmed_poll_leaves_a_completed_death_registered`). The cheapest form
  leaves the poll alone: a round serves a mutator it has not served for X at a
  threshold of one, the collector already walking every record and already
  keeping a timer. X is not a crate constant — it comes from the pressure path
  or from an rfc ABI (`dev/DECISIONS.md`, "the safepoint poll takes the free
  path's road").
- [ ] **A bounded sweep of R, when a workload asks for it.** Built and taken
  out on 2026-09-18 (commit `0ceb364`, reverted the same day): `BlockSweep`
  over one block of the ring, retiring completed deaths in place, unlinking a
  block it empties — a peek reads two blocks, and a hole before the records
  answers a batch of nothing — and bringing the block words together over a
  ring left empty, or `has_at_least` offers a batch every round. Three Critic
  findings are in that commit's tests. Not built because the `POSTED` fire
  already compacts R whole and the unarmed poll runs no retirement by ruling
  (`dev/DECISIONS.md`, "the safepoint poll takes the free path's road"); the
  hold it would shorten is at most sixty-three dead slots per thread. Comes
  back when a measured workload shows that hold, and then on the free path's
  road.
