...
>
> This was the implementation document as written on 2026-07-03. The
> code has moved a long way since, and this text was allowed to drift
> rather than being updated with it, which is the mistake the project
> rule in `dev/WORKFLOW.md` now exists to prevent.
>
> Known to be wrong here: blocks are 64 KB, not 32 KB; there is no
> `free`/`local_free` two-list scheme; and everything built after this
> date is simply absent — memory categories, the store barrier, the
> arena reset fixpoint with promotion, per-block cross-thread free
> queues, block abandonment and adoption.
>
> Kept because the *reasoning* is still worth reading: it records what
> was considered and why, which the current document does not repeat.
> For how the crate works today, see [`docs/memory-manager.md`](../memory-manager.md).
