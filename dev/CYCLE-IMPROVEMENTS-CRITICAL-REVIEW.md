...
> Состояние на 2026-09-18: записка остаётся анализом кандидатов, ни один из
> которых не принят как изменение алгоритма (`dev/DECISIONS.md`, «analysis
> of a candidate that may be refused stays in `dev/`, and the rfc moves only
> on adoption»). Что изменилось после её даты: перенос trace в фоновый
> поток, против которого записка возражает в разделе «Какие быстрые решения
> отклоняем», построен 2026-09-15 и 2026-09-16 — гонку на чтении cells
> закрывает `cells::AtomicCells`, detach заменён чтением кольца из-за
> писателя (`dev/DECISIONS.md`, «the outbox form is deleted: the collector
> reads the ring behind its writer»); сравнение плотности строк, которого
> просит раздел 4, снято переписью `cycle::census` и её replay
> (`dev/BENCHMARKS.md`, 2026-09-12), а время фаз и перцентили паузы, которых
> просит раздел 1, не измерены. Разбор протокола, на который ссылается
> записка, перенесён в `docs/history/`.
...
[разбор протокола](../docs/history/collector-mutator-memory-review-2026-09-09.md),
