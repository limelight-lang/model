...
- **A collection runs off the poll and off the explicit fire**, and
  ...
  | `worker` | the collector thread — born by `ensure_thread`, registered through `ll_thread_init`, rounding over the records every 10 ms — and the batch it makes for one owner under its token: skip an owner collecting in line, clamp K to P's room, peek up to K entries from behind R's writer, mark and scan them through `cells::AtomicCells` on its own arena under a block budget, post one verdict per root into P in R's order — every root unwalked when the trace met the budget — and advance R past them by a guard that runs from the unwind too; K halves on a budget met and doubles back on a completed batch; no production path births the thread until S49.7 | the tests alone |
