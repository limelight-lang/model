...
- GC C ABI and the safepoint: `src/gc.rs` — the four symbols the
  ...
  reserve, refill the queue's spare segments, drain its overflow buffer, make
  the returns a foreign trace left withheld, re-offer the deferred lane where
  the epoch moved; then, behind the gate, it disposes of P's prefix, fires
  if armed, and last signals the collector its record names.
