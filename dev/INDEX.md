...
- **A collection runs off the poll and off the explicit fire**, and
  ...
  |---|---|---|
  | `queue` | the per-thread candidate queue: the ring R a registration writes into by two plain stores (`ring`, over the record's two words), the deferred lane as a chain of the same blocks spliced back at the epoch's turn, the base block, the spares — which the poll refills where short, first from the block a burst left empty behind R's tail block — and the overflow buffer, and the cell that lends the collection workspace; `compaction` is the in-place pass over all of them at a collection's close, with three destinations — kept in order, the deferred lane and `ll_free`; `verdicts` is the ring P the collector posts into — one block per thread, drawn with the record, never grown — and the owner's side of it: the collection the byte's `POSTED` arms reads P's proposed and unwalked roots as its whole batch (`collect::collect_over_the_verdicts`), the pressure path and the exit read them ahead of R, every close disposes of P whole on every ending and advances it, no poll reads a verdict, and a retirement inside a teardown nulls P's completed deaths in place under `POSTED` unswapped | `refcount::release_word`, `gc`'s poll, `cycle::collect` |
  ...
  | `token` | the per-thread trace token: one byte of five states — `FREE`, `MUTATOR`, `REQUESTED\|s`, `COLLECTOR\|s`, `POSTED` — taken by compare-and-swap, held by the owner from its take through its close on both paths and by a collector for one batch, released by one store, and waited on through a mutex by an owner whose graph a collector is tracing; the exit's claim is the one never released (`rfc/dev/design/trace-token-handshake.md`) | `cycle::collect`, both paths |
  | `mutator_record` | the 256-byte record the token stands in — the token's line, the collector's line with R's front block and P's tail block, the owner's line with R's tail block, P's front block and the collecting word, and the hold line, under which a collector reads the rings' blocks before its claim and which an exit finding it set leaves the ring to — carved from GC-metadata blocks the process keeps and reused through a free list, held while nobody lives in it, and not handed out while a hold or a left ring stands; P's one block is drawn with the record and goes back with it, or with the reading's hand-back | `memory::heap::ll_thread_init` and `ll_thread_exit`, `cycle::token`, `cycle::queue`, `cycle::collect` |
  | `worker` | the collector threads — the elder born by `ensure_thread` at the poll's first wake and at a pressure collection's ending, registered through `ll_thread_init`, waiting between rounds on a fallback timer that returns to its minimum after a batch or a freeing disposition, holds over work it could not take and doubles after an empty round up to a maximum, and woken early by an owner's poll when a registration filled a block of its R, by an owner's consent to a standing request and by a pressure collection; siblings born into slots under the embedder's cap (`ll_gc_set_collector_cap`) by a collector whose backlog outlasted two rounds, given every second owner by the word in the record that names its collector, and ended by the elder when idle, their owners named back to it — and the round over the records named to a collector, which serves each owner whose R the collector reads at or above that threshold off the front block alone, and the batch it makes for one owner under its token: request the byte and wait a bound for the owner's consent — a silent owner's request stands on the thread's frame and is served at the next checkpoint — skip an owner whose byte is not free, clamp K to P's room, peek up to K entries from behind R's writer, mark and scan them through `cells::AtomicCells` on its own arena under a block budget, post one verdict per root into P in R's order — every root unwalked when the trace met the budget — advance R past them by a guard that runs from the unwind too, and release to `POSTED`, which the owner's next slot free or poll reads as its trigger for the collection over P; K halves on a budget met and doubles back on a completed batch | `cycle::collect`'s pressure path, `gc`'s poll through `cycle::queue` |
  ...
  | `ring` (`src/ring.rs`) | the single-producer single-consumer ring of pool blocks the candidate queue is built on, in moodycamel's `ReaderWriterQueue` form: `Writer` for the one producer, `Reader` for the one consumer with a peek/commit pair, `Quiescent` for the owner's walk and in-place pack while no reader runs, `Chain` for a lane the owner fills alone and splices in whole | `cycle::queue` |
...
- Owner candidate retirement: `cycle/queue/compaction.rs` combines two bounded
  ...
  after membership and shadow readers end. `queue/tests/mutator_retirement.rs`
...
- GC C ABI and the safepoint: `src/gc.rs` — the four symbols the
  ...
  `ll_gc_checkpoint`, `ll_gc_checkpoint_ack`) and the embedder's dial over
  the collector threads (`ll_gc_set_collector_cap`). The two collecting
  entries run `cycle::collect`'s collection off the poll. The poll has its
  duties before it, in order: refill the log reserve, refill the critical
  reserve, refill the queue's spare segments, drain its overflow buffer, read
  the token byte through the one reading the slot free entry shares — a
  request consented to, `POSTED` arming the collection over P — make the
  returns a foreign trace left withheld, re-offer the deferred lane where
  the epoch moved; then, behind the gate and unless a collector is tracing,
  it fires what the arming word names (`Arming`: P alone, or R whole), notes
  a freeing disposition for the collector's timer, and last signals the
  collector its record names, birthing the elder at the first wake.
...
- Entity tracing: `src/cells.rs` — the kind-dispatched tracer
  ...
  and `cycle::worker`'s batch traces through `AtomicCells`
