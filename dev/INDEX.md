...
- **A collection runs off the poll and off the explicit fire**, and
  ...
  | `owner_record` | the 256-byte record the token stands in — the token's line, the collector's line with R's front block and P's tail block, the owner's line with R's tail block, P's front block and the collecting word, a spare — carved from GC-metadata blocks the process keeps and reused through a free list, held while nobody lives in it; P's one block is drawn with the record and goes back with it | `memory::heap::ll_thread_init` and `ll_thread_exit`, `cycle::token`, `cycle::queue`, `cycle::collect` |
