...
| Module | Responsible for | Knows | Does not know | Depends on |
|---|---|---|---|---|
| `gc` | the GC C ABI and the safepoint: `ll_gc_collect_cycles`, `ll_gc_maybe_collect`, `ll_gc_checkpoint`, `ll_gc_checkpoint_ack`, and the poll's four duties in order — refill the log reserve, refill the critical reserve, refill the queue's spare segments, drain its overflow buffer | that the checkpoint bracket is emitted in every build, collector or none | the arming policy (compiler's); any collector's internals — the two collecting entries report zero until S36.7 | `reserve`, `critical`, `cycle/queue` |
...
again; `reclamation` severs, frees and drops. What is missing between them is
the caller: S36.7 wires the collection into the ABI, and S36.6 writes the
maturation stamp. The design is `rfc/model/gc/rc-cycle.md`,
and this section becomes a path when a program can run one.
