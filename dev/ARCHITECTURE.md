...
| `heap → static_block`, `weak`, `cycle::queue`, `cycle::parking` | `ll_thread_exit`, `ll_thread_init` | thread exit owns the order its per-thread state dies in, because TLS destructor order is unspecified and puts the exit guard last (decision 2026-08-03). These are disposal calls only: `heap` learns nothing about cells or verdicts, it names `dispose`-shaped functions in a fixed sequence. The parking list is disposed before the heaps that allocated it; the queue joins at both ends — its cells are filled beside the two memory reserves at init, and its segments go back before theirs at exit |
...
**Where the collector's duty sits.** A dying enrolled slot owes the
...
| `memory/critical` | the eight-block per-thread critical reserve, the second of `exceptions.md`'s three and separate from the first so neither consumer's worst case is the sum; drawn only after the pool refuses, and `give_back` refills it before a returned block reaches the pool | block pool; that a returned block is the reserve's before it is the pool's | who draws and what for — the collection's arena decides its own order. Two customers draw today, the arena and the enrolment queue; the mutator that cannot collect arrives with S38.4 | `block_pool` |
...
| `memory/stdapi` | the size-less allocator front door: `ll_malloc`/`ll_free`/`calloc`/`realloc`/aligned + `GlobalAlloc`; routes `ptr & !BLOCK_MASK` → header `kind`; asks `refcount::is_enrolled` and `cycle::parking::park_if_active` before a physical return | every block kind's free route; the heap's `MAX_SMALL`; that an enrolment entry or an open trace withholds a physical return | entity semantics; who its callers are | `block_pool`, `heap`; upward: `refcount` (`is_enrolled`) and `cycle::parking` (`park_if_active`), the two withholding tests on the free path |
...
| Module | Responsible for | Knows | Does not know | Depends on |
|---|---|---|---|---|
| `gc` | the GC C ABI and the safepoint: `ll_gc_collect_cycles`, `ll_gc_maybe_collect`, `ll_gc_checkpoint`, `ll_gc_checkpoint_ack`, and the poll's four duties in order — refill the log reserve, refill the critical reserve, refill the queue's spare segments, drain its escrow | that the checkpoint bracket is emitted in every build, collector or none | the arming policy (compiler's); any collector's internals — the two collecting entries report zero until S36.7 | `reserve`, `critical`, `cycle/queue` |
...
| `cycle` | `rc-cycle` as `PLAN.md`'s S34 through S40 build it. Nine modules today: the per-thread enrolment queue (`queue`), a chain of 64 KiB pool segments a non-final decrement writes into, growing by a pointer swap out of two spare cells the safepoint poll fills and by the critical reserve when both are empty, so the release path neither allocates nor locks — and below the reserve an escrow that cannot refuse, so the write cannot fail either, its storage being one block (the floor) drawn at `ll_thread_init` and returned at thread exit, whose refusal is a thread that never starts; the in-line trace's physical-reuse window (`parking::TraceWindow`), which owns the shadow arena so its close nulls every row before replaying out-of-band returns through `ll_free`, across slotted, retained and both large-entity populations; which shadow row a traced edge lands on (`row::edge_to`), a dispatch on the block's kind because the GC heap holds three populations and only one of them has a stride; the row itself and a block's array of them (`shadow`), two bits of colour over thirty of working count, reserved per touched block and zeroed a group of eight at a time; and the collection's working memory (`arena::ShadowArena`), a bump over pooled blocks that asks the ordinary door first and the critical reserve on refusal, reserves and enrols a block's rows in one allocation at its first touch, returns every block at its end and on its abort alike, and nulls the shadow pointer of every block it stamped before it does; and the trace itself (`mark`), which meets a root's row from its refcount, expands each met entity through `cells::trace_cells`, subtracts one from the row of every child `row::edge_to` places inside the GC heap, and descends only on the meeting's first reach — writing into no entity, so a refused door aborts the collection with the heap byte-identical, and carrying its worklist (`stack`) as a chain of 512-entry segments out of the same arena, forgotten whole when the arena resets; and the verdict over those rows (`scan`), which colours a row above zero live and spreads that colour to everything reachable from it, condemns a row at zero that no live row reaches, and raises a condemned row when a live one reaches it later — reading every row through `arena::met_row`, which allocates nothing and answers with none for a group this collection never zeroed; and the judgement over one component (`exact`), which re-reads every member's refcount on the owning thread and matches their sum against the edges the members hold of each other, discounting one guard per member for the re-verify — dropping the component whole on a member that reads zero, reading no row, and writing nothing at all | which block kinds carry rows; that a retained block places an occupant by its position in the reset's index and a large entity by being alone in its block; that an edge out of the GC heap is an external live reference rather than an error; what a row means before the trace has met it; that a component reaches the judgement as its own member list, the rows having gone back with the arena at the token's release | strides, size classes and slot occupancy, which are `heap`'s; entity kinds and out-edges, which are `cells`' and which it asks that module for rather than dispatching on itself; how the index it searches was built, which is `promote`'s; what a queue entry means, the queue holding entity pointers it never dereferences | `memory/block_pool`, `memory/heap`, `memory/retained`, `memory/large_entity`, `memory/critical`, `memory/stdapi`, `refcount`, `cells`, `gc` |
| `promote` | arena death with promotion (retention only): the destructor/escapee fixpoint, internal-edge counting, in-place category rewrite to GcHeap, `BLOCK_KIND_RETAINED` stamping over a nulled collector line, the survivor list handed on as each retained block's object index, the release-at-reset log | escapee hold-count semantics; the retained block kind; that a block it publishes as retained carries no shadow rows | copying / evacuation (future); who mounted the arena; how the index is read; what the collector puts in that word afterwards | `arena`, `block_pool`, `heap` (the collector line it publishes null), `object`, `refcount`, `weak`, `retained`, `array/entity` (a survivor's storage carries out with it) |
...
| Enrolment queue floor and segments | one 64 KiB block per thread for the life of the thread, plus its segment chain | `cycle::queue` | `refcount::release_word` writes entries; the `ll_gc_maybe_collect` poll fills the spares and drains the escrow; `memory::gc_metadata` counts the blocks |
| Shadow rows and the collection's bump | 64 KiB blocks for the length of one collection | `cycle::arena::ShadowArena` | `mark`, `scan` and `stack` read and write rows out of it; `memory::gc_metadata` counts the blocks |
| In-line trace parking list | TLS, no drop glue | `cycle::parking::TraceWindow` | `stdapi::ll_free` appends physical returns while mark or scan may still address a row; the window replays after its owned arena resets |
...
on each block's header line already), the in-line owner's trace parking
...
the queue entry through `ENROLLED`, and `cycle::parking::TraceWindow`
...
order — a non-final decrement enrols the entity (`refcount` → `queue`);
a trace opens its window and its shadow arena (`parking`, `arena`);
`mark` subtracts the edges the heap holds of itself and `scan` colours
what survives; `exact` re-reads the members on the owning thread before
