...
`cycle::owner_record`), and the three words beside it that carry the worker's
handoff: the outbox the owner's poll fills on a request, the inbox the
collector thread posts to, the request word the thread's round sets, and the
shortage note the owner's pressure path leaves for that round to relay
(`cycle::worker`).
...
`cycle::worker`, and its proposal is collected at the owner's poll.
...
9. **Death is owner-bound.** Every free and every weak-table touch runs
   ...
   thread's proposal is marks on the entries of a chain the owner's poll
   collects as its batch (`cycle::worker`). Collections run from the poll, explicit fire and
