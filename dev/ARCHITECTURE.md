...
list (S36.2), and an owner-addressable trace token and handoff in S38.
...
- bits 16–19, the **maturation stamp** — the epoch a commit wrote it in at
  ...
  thread's commit, over a component its exact validation read as externally
  referenced (`cycle::finalization`), and it writes both fields with one
  ...
  which retires a stamp with no pass to clear it. The reader of record is
  S37.1's descent, which stops at a mature edge target;
