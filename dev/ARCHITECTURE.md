...
| `cycle` | `rc-cycle` as `PLAN.md`'s S33 through S40 build it. Two things today: which shadow row a traced edge lands on (`row::edge_to`), a dispatch on the block's kind because the GC heap holds three populations and only one of them has a stride; and the collection's working memory (`arena::ShadowArena`), a bump over pooled blocks that asks the ordinary door first and the critical reserve on refusal, returns every block at its end and on its abort alike, and nulls the shadow pointer of every block it stamped before it does | which block kinds carry rows; that a retained block places an occupant by its position in the reset's index and a large entity by being alone in its block; that an edge out of the GC heap is an external live reference rather than an error | strides, size classes and slot occupancy, which are `heap`'s; entity kinds and out-edges, which are `cells`'; how the index it searches was built, which is `promote`'s | `memory/block_pool`, `memory/heap`, `memory/retained`, `refcount` |
