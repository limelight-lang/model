...
| In-line trace withheld returns | the workspace's fixed region for the first 1,024 records and nothing besides; TLS holds one non-owning pointer to the control line inside that region, and there is no drop glue | `cycle::deferred_slot_reuse::ActiveTrace` | `stdapi::ll_free` appends physical returns while mark or scan may still address a row; the window replays after its owned arena has swept its rows, then walks the slots its marks stacked and the blocks its marks listed, and the arena gives its own blocks back after all of that — so an unwind out of that hand-back still finds every return made; `memory::gc_metadata` counts nothing of it, the region being the workspace's |
...
- bit 15, `DEAD_IN_PLACE` — the entity died inside a trace window whose
  ...
  took it. A slot of a block the marking thread does not own is the one that
  goes on a stack instead of listing its block, threaded through the dead
  slots themselves at the offset a free list would use: the marking thread
  may not walk such a block, its bump cursor being the owner's to move, and
  the close returns each stacked slot through `ll_free`. The mark carries no
  owner, so a block walk cannot tell a foreign window's mark from its own —
  which is one more clause the one-window premise carries and S38's token
  closes. A death in memory the
