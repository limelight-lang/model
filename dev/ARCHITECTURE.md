...
|---|---|---|
| `object → gc` | `ll_release_vector` | the safepoint bracket a batched run pays: `ll_gc_checkpoint_ack` before it, `ll_gc_checkpoint` after (decision 2026-07-28). Both bodies remain empty because the built in-line collector needs no handshake; the future accelerator owns their final contract |
...
|---|---|---|---|---|
| `memory/block_pool` | 2 MB OS regions carved into size-aligned 64 KB blocks; global free chain (mutex) + per-thread cache; region registry | OS allocation; the `BlockHeader` base fields | what any payload contains; entities, classes, GC | — (bottom; its `heap`, `reserve` and `cycle::queue` references are the shared test-lock harness only: the guard starts a thread and draws its collection workspace so that a case counting blocks counts its own) |
...
| `memory/critical` | the eight-block per-thread critical reserve, the second of `exceptions.md`'s three and separate from the first so neither consumer's worst case is the sum; drawn only after the pool refuses, and `give_back` refills it before a returned block reaches the pool | block pool; that a returned block is the reserve's before it is the pool's | who draws and what for — the collection's arena decides its own order. Two customers draw today, the arena and the candidate queue; the mutator that cannot collect is answered null and draws nothing (`PLAN.md`, Fog, "The threshold arming policy and the collector-thread accelerator") | `block_pool` |
| `memory/retained` | the survivor list of each retained former-arena block — sorted, in the arena's own memory, named by the block's header line — and the atomic count word that returns the block: held occupant slots (live survivors and dead candidates awaiting owner retirement), pinned payloads and the lists of other blocks standing in it; published by the reset, read by the trace and the test-only enumerator, no process-global table and no lock | block addresses and arrays of addresses, plus the one question it asks through them: whether a slot still holds an allocation the block must retain, which `refcount::slot_state` and the candidate bit answer | what lives at those addresses — entities, classes, verdicts; where the arena placed the list | `block_pool` (stamping an emptied block and handing it over), `heap` (the collector line's words), `refcount` (`slot_state`, the occupancy and held-identity test) |
...
| Module | Responsible for | Knows | Does not know | Depends on |
|---|---|---|---|---|
| `gc` | the GC C ABI and the safepoint: `ll_gc_collect_cycles`, `ll_gc_maybe_collect`, `ll_gc_checkpoint`, `ll_gc_checkpoint_ack`, and the poll's five duties in order — refill the log reserve, refill the critical reserve, refill the queue's spare segments (from the block R has to spare first), drain its overflow buffer | that the checkpoint bracket is emitted in every build, collector or none | the arming policy (compiler's); any collector's internals — the two collecting entries call `cycle::collect` and read nothing of it | `reserve`, `critical`, `cycle/queue` |
...
| `cycle` | `rc-cycle`, the in-line collection whole and the collector thread beside it; the maturation prune's corpus figures are `PLAN.md` S37's. The modules named below stand beside `trace`, which runs the two phases over one batch: the per-thread trace token (`token`), taken by compare-and-swap before the ring is read and released by one store after the trace's last row read — the scan's end off the poll, the harvest sweep under pressure — and so before the exact validation and the first destructor on both paths, with a mutex an owner blocks on when a collector holds it, and taken only after the eligibility test, so a thread that may not collect never waits, the exit's claim being the one never released; the record the token stands in (`mutator_record`), four 64-byte lines per thread — the token's, the collector's words of the two rings, the owner's words of them with the collecting word that keeps a collector out of an in-line collection for its whole length, and the hold line the collector sets around its pre-claim reading of the rings' blocks, which an exit reaching a ring's return under it leaves to the reading's hand-back — carved from GC-metadata blocks the process keeps and reused through a free list, so that a worker's load of it under no claim reads memory that outlives the thread, and held while nobody lives in it; the collector threads and their round over one owner (`worker`) — the elder born at a pressure collection's ending through `ll_thread_init` like any registered thread, so a refused floor is a thread that never started and a later birth call births again, and siblings the elder births into slots under the embedder's cap when a backlog outlasts two rounds, each owner named to one collector by a word in its record and the idle sibling ended by the elder; woken by an owner's poll when a registration filled a block of that owner's R, by a pressure collection, and by a fallback timer adapted between two bounds, each walks the records named to it and makes a batch under the token of each owner whose ring it reads at or above the threshold — up to K entries peeked from behind the owner's writer and clamped to P's room, traced through `cells::AtomicCells` on the collector's own arena under a block budget, one verdict per root posted into P in R's order, and R advanced past them by a guard that runs from the unwind too; the segmented record chain the trace's worklist is built on (`records`), which allocates nothing and takes its segments from whichever owner holds them; the per-thread candidate queue (`queue`), a ring of 64 KiB pool blocks in `ring`'s form that a non-final decrement writes into by two plain stores, read behind the writer by whoever holds the trace token — the in-line collection reads it whole as its batch and compacts it in place at its close, drawing nothing — growing by a block linked in out of two spare cells the safepoint poll fills, by the critical reserve when both are empty, and by the circle's own consumed blocks — one of which the poll unlinks back into a short cell when a burst has left it empty behind the tail block — so the release path neither allocates nor locks; its deferred lane a chain of the same blocks the owner fills at a close and splices back into the ring at the epoch's turn with no copy — and below the reserve a bounded overflow buffer that cannot refuse, so the write cannot fail either, its storage being one block (the base block) drawn at `ll_thread_init` and returned at thread exit, whose refusal is a thread that never starts — and, in that block's control line, the address of the second block a thread holds for its life, the collection workspace, whose rule is the opposite one: drawn at the thread's first collection through the ordinary allocation path alone, and a refusal there is a collection that does not start; the in-line trace's physical-reuse window (`deferred_slot_reuse::ActiveTrace`), which owns the trace scratch arena so its close nulls every row before making its out-of-band returns through `ll_free`, across slotted, retained and both large-entity populations, and whose control line is the whole of the workspace's fixed region, its withheld returns standing in the dying entities themselves, so that no path of it asks an allocation path and only a thread's first collection can be refused — a death is returned at once where this collection never met its block, and otherwise marked in the entity's own header and pushed onto one stack through the word a free list would use, which the close pops; which shadow row a traced edge resolves to (`row::resolve_edge_target`), a dispatch on the block's kind because the GC heap holds three populations and only one of them has a stride; the row itself and a block's array of them (`shadow`), two bits of colour over thirty of working count, reserved per touched block and zeroed a group of eight at a time; and the collection's working memory (`arena::TraceScratchArena`), a bump that opens over the thread's workspace behind the two fixed regions at that workspace's head, 56,960 bytes of it and grows past it into pooled blocks, asking the ordinary allocation path first and the critical reserve on refusal, reserves a block's rows and attaches them to the touched list in one allocation at its first touch, returns every block it drew at its end and on its abort alike while rewinding over the workspace it only borrowed, and nulls the shadow pointer of every block it stamped before it does; and the trace itself (`mark`), which reads a root's refcount before its cells and expands nothing at zero — an entity the queue still names after its teardown carries cells naming slots the allocator may have handed on — ensures a root's row from that refcount, expands each entity through `cells::trace_cells`, subtracts one from the row of every child `row::resolve_edge_target` places inside the GC heap, and descends only on the first visit — stopping instead at an edge target whose stamp carries this collection's epoch at the traversal age threshold and which no candidate queue names, which is the one rule in this design that bounds the closure and costs recall until the epoch turns over — writing into no entity, so a refused allocation aborts the collection with the heap byte-identical, and carrying its worklist (`stack`) in 256-entry segments out of the same arena, one at the first push, each entry the pair of an entity and the shadow row its meeting found, forgotten whole when the arena resets; and the verdict over those rows (`scan`), which colours a row above zero live and spreads that colour to everything reachable from it, colours a row at zero that no live row reaches potentially unreachable, and raises such a row when a live one reaches it later — reading every row through `arena::find_initialized_row`, which allocates nothing and answers with none for a group this collection never zeroed; and the members a pressure collection takes out of its rows (`members`), a control line and 1,024 records in the second fixed region of the workspace, written by the sweep that nulls the blocks' shadow pointers and read by the teardown that runs after every block has gone back — a trace's whole set or none of it, an overflow emptying the list rather than handing back a part of a set whose remaining members still name it; and the exact validation of one component (`validation`), which re-reads every member's refcount on the owning thread and matches their sum against the edges the members hold of each other, subtracting one guard reference per member for the re-verify — dropping the component whole on a member that reads zero, reading no row, and writing nothing at all; and the two writes that answer stands for (`finalization`), a guard reference on every member of a confirmed component and then every weak cell naming one nulled, both inside the call that validates, so that a component the exact validation refuses is left untouched — and, within one finalization, no destructor runs over a member of another component whose cell still resolves, `seal` being the only source of the value the destructor pass takes; then the pass itself, which runs each member's `__destruct` once behind the gate the death path dispatches on and carries one reading of whether any of them ran, and the revalidation behind it, which reads a component again with one guard reference per member subtracted, keeps it for the teardown or gives its members their true counts back through the counted release — and answers with a value borrowing the revalidation, so that the teardown of the component it names runs before the next component is read; and stamps every component either reading proves live with the commit's epoch and one age more than its youngest member carried, which is the one write of a collection that outlives it, and the count of closed commits the epoch is read off (`epoch`), one process-global word advanced where the commit closes; and the teardown that acts on the answer (`reclamation`), which takes room for a component's displaced children before it empties the first cell, cuts every internal edge through a narrow counter store and queues every external child, lets the ordinary death path free each member — into a withheld slot where the queue still names it, no bit being cleared here — and drops the queued children in the order the sever displaced them once the last free is behind — its queue a second chain over the arena's bump, emptied at the end of every component, and its refusal a component left whole with its guards off; that a whole commit uses one finalization is the driver's, and nothing in the module holds it | which block kinds carry rows; that a retained block places an occupant by its position in the reset's index and a large entity by being alone in its block; that an edge out of the GC heap is an external live reference rather than an error; what a row means before the trace has met it; that a component reaches validation as its own member list on the pressure path, the rows having gone back with the arena, and as the rows themselves on the path off the poll, the token's release ending the right to trace and not the window | strides, size classes and slot occupancy, which are `heap`'s; entity kinds and out-edges, which are `cells`' and which it asks that module for rather than dispatching on itself; how the index it searches was built, which is `promote`'s; entity layout inside a queue entry; owner retirement reads only its count and free mark through `refcount` | `memory/block_pool`, `memory/heap`, `memory/retained`, `memory/large_entity`, `memory/critical`, `memory/stdapi`, `memory/gc_metadata`, `memory/barrier`, `refcount`, `cells`, `object`, `weak`, `gc`, `ring` — re-enumerated 2026-09-07 from the `crate::` paths rather than patched, which is what `dev/WORKFLOW.md`'s fourth pass asks for: `object` and `weak` came in with the destructor pass and the weak invalidation, `memory/barrier` with the teardown's deferred drops, `ring` with the candidate ring (2026-09-16) |
...
open-gate poll (`cycle::queue::verdicts`).
...
15. **One trace at a time per mutator thread**, the claim being that
    ...
    with the premise it rested on. The claim word is `cycle::token`'s.
