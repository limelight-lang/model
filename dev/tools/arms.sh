...
# interleaved inside each repeat, one process a cell, the rig's line with the
# arm and the repeat in front. The deciding loads are paced as the S65.24
...
for repeat in $(seq 1 "$REPEATS"); do
    for placement in spare-core shared-core; do
        if [ "$PHASE" = deciding ]; then
            for spec in live-churn:1 deferred-then-dead:1 deferred-live-large:1 \
                        garbage-25:15 registered-ring-live:15; do
                for arm in $ARMS; do
