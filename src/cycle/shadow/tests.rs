...
/// Bytes an array of `slots` rows needs, over memory this module can
...
/// together.
fn dirty_buffer(slots: u32) -> Vec<u64> {
    let words = bytes_for(slots).div_ceil(size_of::<u64>());
    vec![0xEEEE_EEEE_EEEE_EEEE; words]
}

/// The array at the head of `buffer`, initialised for `slots` rows.
///
...
fn array_over(buffer: &mut [u64], slots: u32) -> *mut RowArray {
