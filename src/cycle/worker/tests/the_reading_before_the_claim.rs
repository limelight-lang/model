...
//! a mutator that exits between the take and the loads leaves R's blocks and
...
//! mutator thread the case spawns; the case's act runs on this thread inside
...
/// A mutator on a thread of its own, with one registered candidate so that R
...
fn mutator_waiting_to_exit() -> ExitingOwner {
    let (go, wait) = mpsc::channel::<()>();
    let (report, exited) = mpsc::channel::<()>();
    let (tell, record) = mpsc::channel::<Sent<*mut MutatorRecord>>();
    let thread = std::thread::spawn(move || {
        ...
        let class = ClassBuilder::new("exiting_mutator_root").build();
        ...
        let _ = wait.recv();
        // Before the exit, so that nothing of the mutator's returns memory
    ...
    let record = record
        .recv()
        .expect("the mutator reported its record")
        .into_inner();
    ExitingOwner {
        record,
        go,
...
fn blocks_left_in(record: *mut MutatorRecord) -> usize {
...
fn served_with_an_act_inside_the_reading(
    ...
    let served = unsafe { testing::serve_alone(record) };
    testing::confine_rounds_to(std::ptr::null_mut());
    served
}

#[test]
fn an_mutator_exiting_under_the_reading_leaves_its_blocks_to_the_hand_back() {
    let _g = test_guard();
    let ExitingOwner {
        record,
        go,
        exited,
        thread,
    } = mutator_waiting_to_exit();
    assert!(blocks_left_in(record) >= 2, "R holds a block and P its one");

    // Inside the reading, on this thread: the mutator exits to the end while
    ...
    let served = served_with_an_act_inside_the_reading(
        ...
        Box::new(move || {
            ...
            exited.recv().expect("the mutator exited");
    ...
    assert!(
        ...
        "nothing to serve of an exited mutator"
    );
    assert_eq!(
    ...
    assert!(mutator_record::registry_lists_free(record));
    assert!(
        !unsafe { mutator_record::take_for_reading(record) },
        "a record whose rings the hand-back returned is on the free list and refuses a reading"
    );
    thread.join().expect("the mutator finished");
    mutator_record::pin_for_test(record, false);
}
...
fn a_thread_asking_for(record: *mut MutatorRecord) -> bool {
    ...
    std::thread::spawn(move || {
        ...
        mutator_record::take_this_record_for_test(wanted);
        assert!(
            crate::memory::heap::ll_thread_init(),
        ...
        if got {
            assert!(
                mutator_record::lines_are_fresh(wanted),
...
fn the_registry_hands_the_record_out_only_after_the_hand_back() {
    ...
    let served = served_with_an_act_inside_the_reading(
        ...
        Box::new(move || {
            ...
            exited.recv().expect("the mutator exited");
            assert!(
                mutator_record::registry_lists_free(record),
...
fn a_take_between_the_exits_load_and_its_store_is_still_left_the_blocks() {
    ...
    mutator_record::at_the_next_exits_hold_check(Box::new(move || {
