...
/// waits, and one collector's list is never threaded through the record's
/// next life (`dev/design/the-standing-request-lives-on-the-record.md`).
#[test]
fn a_record_standing_in_a_collectors_list_is_not_handed_out_until_unlinked() {
    let _g = test_guard();
    let (_token, record, release, thread) = a_started_thread();
    pin_for_test(record, true);
    drop(release);
    thread.join().expect("the first thread exited");
    assert!(registry_lists_free(record));
...
fn a_thread_living_twice_takes_a_record_per_life() {
    ...
    let (first, second) = std::thread::spawn(|| {
        ...
        unsafe {
            (*second)
                .token
                .release_claim(crate::cycle::worker::ELDER, false)
        };
