...
impl<'a> Quiescent<'a> {
    ...
    /// Hand `visit` every block of the circle from the front block to the
    /// tail block, in order, for a caller that walks the blocks rather than
    /// the entries.
    pub(crate) fn blocks_in_order(&self, mut visit: impl FnMut(*mut BlockHeader)) {
    ...
    /// Bring the tail block back to the front block where the circle holds no
    /// entry, and answer whether it did.
    ///
    /// This is the shape a whole-ring pack leaves ([`Packing::drop`]), and a
    /// reader's pre-claim reading depends on it: [`Reader::has_at_least`]
    /// answers true for any `front_block != tail_block`, taking that for
    /// entries standing in a block between them. An owner that empties the
    /// ring block by block moves neither word, so without this a swept-empty
    /// ring would offer a collector a batch every round for the rest of the
    /// thread's life
    /// (`tests::a_ring_swept_empty_reads_empty_to_a_collector`).
    ///
    /// The blocks between the two stay in the circle, empty, for the writer's
    /// next round, as they do after a pack.
    ///
    /// # Safety
    /// No reader runs and no reading of this ring stands uncommitted.
    pub(crate) unsafe fn collapse_if_empty(&self) -> bool {
        let front_block = self.front_block();
        if front_block.is_null() || front_block == self.tail_block() || self.count() != 0 {
            return false;
        }

        self.0.tail_block.store(front_block, Ordering::Relaxed);
        true
    }

    /// Take `block` out of the circle and answer it, for a block that holds
    /// no entry and stands between the front block and the tail block. Null
    /// where it is one of those two, where it holds an entry, or where the
    /// circle is one block: the front block is where a reader stands and the
    /// tail block is where the writer appends, and the ring's own rules
    /// carry an empty block in either place.
    ///
    /// The predecessor is found by a walk from the front block, the circle
    /// being linked one way; a circle carries few blocks, and the alternative
    /// is a back link on every one of them.
    ///
    /// # Safety
    /// `block` is a block of this circle, and no reader runs.
    pub(crate) unsafe fn unlink_empty_block(&self, block: *mut BlockHeader) -> *mut BlockHeader {
        let front = self.front_block();
        let tail = self.tail_block();
        if block.is_null() || block == front || block == tail {
            return std::ptr::null_mut();
        }

        let b = ring(block);
        let empty = unsafe {
            (*b).reader.front.load(Ordering::Relaxed) == (*b).writer.tail.load(Ordering::Relaxed)
        };
        if !empty {
            return std::ptr::null_mut();
        }
        ...
        while unsafe { (*ring(previous)).link.next.load(Ordering::Relaxed) } != block {
            previous = unsafe { (*ring(previous)).link.next.load(Ordering::Relaxed) };
            if previous == front {
                return std::ptr::null_mut();
            }
        }

        let after = unsafe { (*b).link.next.load(Ordering::Relaxed) };
        unsafe {
            (*ring(previous)).link.next.store(after, Ordering::Release);
            (*b).link
                .next
                .store(std::ptr::null_mut(), Ordering::Relaxed);
        }
    ...
    /// Open a sweep over the entries of `block` alone ([`BlockSweep`]), for a
    /// caller whose bound is one block rather than the ring.
    ///
    /// # Safety
    /// `block` is a block of this ring's circle.
    pub(crate) unsafe fn sweep_block(&self, block: *mut BlockHeader) -> BlockSweep<'_> {
        BlockSweep::open(self, block)
    }
...
/// One sweep over a single block of a ring: the entries between that block's
/// front and tail, read in order and answered for one at a time as
/// [`Packing`]'s are, with the kept ones packed from the same front and the
/// block's tail lowered to what was kept.
///
/// **The rest of the circle stands untouched**, the ring's front and tail
/// block words with it, so a middle block this leaves half-empty is read as
/// it stands: a block's entries are bounded by its own two indices and by
/// nothing else, and the writer appends to the tail block alone.
///
/// **An unwind out of the caller's frame leaves the block whole**, on
/// [`Packing`]'s terms: the entry in hand, if the caller had not yet answered
/// for it, is kept as it stood, and so is every entry not yet read.
pub(crate) struct BlockSweep<'a> {
    ring: &'a Quiescent<'a>,
...
impl<'a> BlockSweep<'a> {
    fn open(quiet: &'a Quiescent<'a>, block: *mut BlockHeader) -> Self {
        let (front, tail) = if block.is_null() {
            (0, 0)
        } else {
            let b = ring(block);
            unsafe {
    ...
    /// Take the next entry of the block in hand, or `None` past its tail. An
    /// entry still in hand from the last read is kept as it stood.
    pub(crate) fn read(&mut self) -> Option<usize> {
        if self.holding {
            self.keep();
        }

        if self.block.is_null() || self.read == self.tail {
            return None;
        }
    ...
    /// Write `kept` at the write cursor, in place of the entry in hand. The
    /// cursor never passes the read cursor, so no unread entry is written
    /// over.
    pub(crate) fn write(&mut self, kept: usize) {
        self.holding = false;
...
impl Drop for BlockSweep<'_> {
    fn drop(&mut self) {
        if self.block.is_null() {
            return;
        }

        // The unwind's case, and a no-op on the return: an entry in hand the
        // caller never answered for is kept, and so is everything not yet
        // read.
        if self.holding {
            self.keep();
        }
        while self.read != self.tail {
            let entry = unsafe { *(*ring(self.block)).slots[self.read].get() };
            self.read = step(self.read);
            self.write(entry);
        }

        unsafe { self.ring.set_tail(self.block, self.write) };
    }
}
