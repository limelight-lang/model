...
pub unsafe extern "C" fn ll_arena_reset(ctx: *mut LLContext) {
    ...
    // The refusal count is dropped here. The journal records every emptied
    // edge (`KIND_ARENA_RESET_SEVERED_EDGE`) and does not separate the
    // refused child from the one its entry took with it, so the figure is
    // the return's alone; the channel that would carry it to the host is
    // `rfc/runtime/exceptions.md`, "The pending channel", which this crate
    // does not have.
    let _refusals = unsafe { crate::promote::arena_reset_full(resolve(ctx)) };
}
