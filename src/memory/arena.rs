...
impl Arena {
    ...
    /// program-visible count wants one of the two entry points that split
    /// at the same bound and take the dedicated-run path above it:
    ...
    ///
    /// An entry point of its own rather than a lifted bound on
    /// [`Arena::alloc`], because that one serves `ll_arena_alloc` straight from
    /// the C ABI, where an entity and a byte buffer are the same request; it
    /// keeps refusing. And not [`Arena::alloc_large`], which allocates through
    /// `ll_alloc` and would stamp the raw-buffer kind on memory the collector
    /// reads as an `RcHeader` (`rfc/model/memory/large-entities.md`).
    ///
    ...
    pub fn finish_reset(&mut self, mut keep_block: impl FnMut(*mut BlockHeader) -> bool) {
        ...
        Self::drain_log(larges, |rec| unsafe {
            ...
            // An entity in a block of its own is the one record here that is
            // not a byte payload, and it dies the way every unpromoted arena
            // entity dies: wholesale, with the memory it sat in. What differs
            // is that its memory leaves through `ll_free`, which asks an entity
            // slot for the refcount-0 header a death leaves behind — and an
            // arena entity is uncounted, so it still carries the count its
            // factory wrote. Zeroing it here is that death, stated where it
            // happens.
