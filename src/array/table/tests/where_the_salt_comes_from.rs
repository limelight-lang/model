...
/// The body of `fn draw_salt`, by brace counting from its first line —
/// which holds because `rustfmt` governs this crate.
fn draw_salt_body(text: &str) -> String {
    ...
    for line in text.lines() {
        ...
        if line.contains("fn draw_salt") && !line.trim_start().starts_with("//") {
            arming = true;
        }
...
/// The draw must reach the per-process key and must not reach the
...
fn draw_salt_derives_from_the_key_module_and_not_the_foldable_seed() {
    let text = fs::read_to_string(Path::new(env!("CARGO_MANIFEST_DIR")).join("src/array/table.rs"))
        .expect("the table source is readable");
    let body = draw_salt_body(&text);
    assert!(!body.is_empty(), "the walk did not find draw_salt at all");
    assert!(
        body.contains("process_key"),
        "draw_salt no longer reaches the per-process key:\n{body}"
    );
    assert!(
        !body.contains("hash_bytes") && !body.contains("seed::"),
        "draw_salt reaches the foldable seed:\n{body}"
    );
}
...
";
    let body = draw_salt_body(planted);
    assert!(
